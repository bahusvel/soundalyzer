...
pub fn metaphone<T: ToString + ?Sized>(word: &T) -> String {
    ...
    let word_char = upper_word.chars().collect::<Vec<_>>();
    ...
    match word_char[0] {
        ...
        'W' if word_char[1] == 'H' => {
            action_when_second_letter_match(&mut local, 'H');
            local.remove(0);
            local.insert(0, 'W');
        }
        'W' => local.push_str(&upper_word),
        'X' => {
            local.push_str(&upper_word);
            local.remove(0);
            local.insert(0, 'S');
        }
    ...
    while (code.len() < MAX_LEN) && n < local_size {
        ...
        if char_at != 'C' && is_previous_char(&local, n, char_at) {
            ...
            match char_at {
                ...
                'C' if c_testing(&local, n, char_next) && region_match(&local, n, "CIA") => {
                    code.push('X')
                }
                'C' if c_testing(&local, n, char_next)
                    && !is_last_char(local_size, n)
                    && FRONTV.contains(char_next) =>
                {
                    code.push('S')
                }
                'C' if c_testing(&local, n, char_next)
                    && is_previous_char(&local, n, 'S')
                    && is_next_char(&local, n, 'H') =>
                {
                    code.push('K')
                }
                'C' if c_testing(&local, n, char_next)
                    && is_next_char(&local, n, 'H')
                    && (n == 0 && local_size >= 3 && is_vowel(&local, 2)) =>
                {
                    code.push('K')
                }
                'C' if c_testing(&local, n, char_next)
                ...
                'P' => code.push(char_at),
                'Q' => code.push('K'),
                'S' if region_match(&local, n, "SH")
                    || region_match(&local, n, "SIO")
                    || region_match(&local, n, "SIA") =>
                {
                    code.push('X')
                }
                'S' => code.push('S'),
                'T' if region_match(&local, n, "TIA") || region_match(&local, n, "TIO") => {
                    code.push('X')
                }
                'T' if !region_match(&local, n, "TCH") && region_match(&local, n, "TH") => {
                    code.push('0')
                }
                'T' if !region_match(&local, n, "TCH") => code.push('T'),
                'V' => code.push('F'),
                'W' | 'Y' if !is_last_char(local_size, n) && is_vowel(&local, n + 1) => {
                    code.push(char_at)
                }
                'X' => {
                    code.push('K');
...
fn g_testing_j(local: &str, n: usize) -> bool {
    !is_last_char(local.len(), n)
        && FRONTV.contains(local.chars().nth(n + 1).unwrap())
        && !is_previous_char(&local, n, 'G')
}

fn d_testing(local: &str, n: usize) -> bool {
    !is_last_char(local.len(), n + 1)
        && is_next_char(&local, n, 'G')
        && FRONTV.contains(local.chars().nth(n + 2).unwrap())
}
...
fn g_testing(local: &str, n: usize) -> bool {
    ...
    (!(last_char && next_is_h)
        && !(!last_char && next_is_h && !is_vowel(local, n + 2))
        && !(n > 0 && (region_match(local, n, "GN") || region_match(local, n, "GNED"))))
}
